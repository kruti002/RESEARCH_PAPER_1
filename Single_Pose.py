...
def cv2_put_text(image, message, position, color):
    cv2.putText(
        image,
        message,
        position,
        ...
        if result:
            ...
            if not err:
                ...
                if probabilities[0, prediction[0]] > 0.6:
                    ...
                    cv2_put_text(flipped, pose_name, (50, 50), (255, 0, 0))
                    ...
                    for i, (text, is_correct) in enumerate(suggestions):
                        color = (0, 255, 0) if is_correct else (0, 0, 255)
                        cv2_put_text(flipped, text, (50, 100 + i * 30), color)
                        print(f"{text} - {'Correct' if is_correct else 'Incorrect'}")

                    # SHAP explanation for the predicted pose only
                    shap_values = explainer.shap_values(df)
                    pose_shap_values = shap_values[prediction[0]]

                    # Display SHAP values for the predicted pose (for debugging)
                    print(pose_shap_values)
                    
                    # Create a summary plot for the predicted pose (optional)
                    shap.summary_plot(pose_shap_values, df, plot_type="bar")
                else:
                    cv2_put_text(flipped, "No Pose Detected", (50, 50), (0, 0, 255))
